...
### 23. Relational Database Schema & Durable Persistence Migration

- **Date:** 2025-07-16
- **Requirement IDs:** REQ-F-005 (owned database design), REQ-NF-004 (ACID, FK integrity, DECIMAL for money, indexes, audit trails)
- **Requirement title:** Relational SQLite schema with durable persistence for all domain entities and portal metadata
- **Files changed:**
  - `src/services/durable-store.ts` (rewritten; full relational DDL with 14 entity tables — `customers`, `accounts`, `transactions`, `transfers`, `yield_configs`, `loan_payments`, `user_defined_fields`, `documents`, `portal_users`, `portal_sessions`, `credentials`, `api_activity`, `audit_log`, `idempotency_records` — foreign key constraints, indexes on tenant_id/account_id/posted_at/timestamp, row-type interfaces, entity-level CRUD methods, portal/credential/activity/audit/idempotency methods, `transaction()` helper)
  - `src/services/tenant-store.ts` (rewritten persistence layer; removed JSON-blob `PersistedTenantDataset`/`serializeTenantDataset()`/`deserializeTenantDataset()`; added ~200 lines of row conversion functions with INTEGER cents via `toCents()`/`fromCents()`; `saveTenantDataset()` writes per-entity rows through `durableStore.saveTenantEntities()`; `loadTenantDatasetFromDb()` reads from relational tables; `ensureTenantDataset()` cascades: relational → legacy JSON blob → baseline clone)
  - `src/services/portal-auth.ts` (migrated from in-memory Maps to SQL-backed persistence; write-through cache with `persistUser()`/`persistSession()` writing to `portal_users`/`portal_sessions` tables; lazy cache loading via `ensureCache()`; `validatePortalSession()` falls back to DB lookup; OTP records remain ephemeral by design)
  - `src/services/developer-registry.ts` (migrated from in-memory Map to SQL-backed persistence; `persistCredential()` writes to `credentials` table; `ensureCache()` loads all credentials on first access; default developer credential persisted to DB)
  - `src/services/api-activity-log.ts` (migrated from in-memory Map to fully SQL-backed; removed `apiActivityByTenant` Map; `persistActivity()` inserts + prunes via SQL; `listApiActivityForTenant()` queries DB directly with SQL WHERE filtering; `captureApiActivity()` now captures `durationMs`)
  - `src/services/audit-log.ts` (migrated from in-memory Map to fully SQL-backed; `writeAuditEntry()` inserts + prunes via SQL; `listTenantAuditEntries()` queries DB with JSON-parsed `details`)
  - `src/services/idempotency-store.ts` (migrated from in-memory Map to fully SQL-backed; `getIdempotentReplay()` queries DB with TTL check; `saveIdempotentResult()` upserts with JSON-serialized payload)
  - `requirements/TRACEABILITY.md` (this entry)
- **Summary:** Replaced the single-JSON-blob-per-tenant persistence model and all in-memory-only portal metadata stores with a proper relational SQLite schema. Core domain entities (customers, accounts, transactions, transfers, yield configs, loan payments, UDFs, documents) are now stored in normalized tables with foreign key relationships (e.g., `accounts.customer_id → customers.id`, `transactions.account_id → accounts.id`). All monetary amounts are stored as INTEGER cents (complying with REQ-NF-004's DECIMAL/NUMERIC requirement) and converted at the persistence boundary via `toCents()`/`fromCents()`. Portal metadata (users, sessions, credentials, API activity, audit entries, idempotency records) that was previously volatile in-memory storage is now durably persisted to SQL tables, surviving process restarts. The write-through cache pattern ensures runtime performance while guaranteeing durability. `PRAGMA foreign_keys` is set outside transactions (SQLite ignores it inside transactions). All 80 tests pass across 39 test files with zero constraint errors.
